...
if ( !function_exists( 'wpdtrt_forms_menu' ) ) {
  ...
  function wpdtrt_forms_menu() {
    ...
    add_options_page(
      ...
      'manage_options',
      'wpdtrt-forms',
      'wpdtrt_forms_options_page'
    );
  }

  add_action('admin_menu', 'wpdtrt_forms_menu');
 ...
 */
if ( !function_exists( 'wpdtrt_forms_options_page' ) ) {

  /**
   * Render the appropriate UI on Settings > WPDTRT Forms
   *
   ...
   *       a. The user's selection: 'wpdtrt_forms_datatype',
   *          so that the dataset can be regenerated after a specified amount of time
   *       b. The data set generated by the API: 'wpdtrt_forms_data'
   ...
   * @since       0.1.0
   */
  function wpdtrt_forms_options_page() {
     ...
     */
    global $wpdtrt_forms_options;

    if ( isset( $_POST['wpdtrt_forms_form_submitted'] ) ) {

      // check that the form submission was legitimate
      $hidden_field = esc_html( $_POST['wpdtrt_forms_form_submitted'] );

      if ( $hidden_field === 'Y' ) {

        // 1. get user preferences from form submission
        $wpdtrt_forms_datatype = esc_html( $_POST['wpdtrt_forms_datatype'] );

        // 2. send user preferences to API to get results
        $wpdtrt_forms_data = wpdtrt_forms_data_get(
          $wpdtrt_forms_datatype
        );

        // 3a. store user preferences in options object
        $wpdtrt_forms_options['wpdtrt_forms_datatype'] = $wpdtrt_forms_datatype;

        // 3b. store API results in options object
        $wpdtrt_forms_options['wpdtrt_forms_data'] = $wpdtrt_forms_data;

        // 3c. store timestamp in options object
        $wpdtrt_forms_options['last_updated'] = time(); // UNIX timestamp for the current time
         ...
         */
        update_option( 'wpdtrt_forms', $wpdtrt_forms_options, null );
      }
    }
     ...
     */
    require_once(WPDTRT_FORMS_PATH . 'views/admin/partials/wpdtrt-forms-options-page.php');
  }
